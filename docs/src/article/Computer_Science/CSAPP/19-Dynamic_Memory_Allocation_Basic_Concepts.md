...
应用程序使用动态分配器(dynamic memory allocators)去操纵虚拟内存，去构造、分配以及释放那些在运行时才知道总大小的虚拟存储器片

栈的总大小是固定的, 内部是动态变化的。默认栈总大小由系统环境决定, Linux 中一般是 8MB, 可以通过编译器参数或者多线程 API 等指定栈总大小

只有堆是纯粹为运行时动态分配而生的, 所以在大多数标准动态内存分配器中，堆是其唯一管理的动态内存区域
...
峰值内存利用率 = 前 k 个请求中最大的聚合有效负载 / 当前堆的总大小: `Uk = max(Pi)/ Hk, i <= k`
