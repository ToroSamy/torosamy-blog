...
包装函数的流程示例：程序 $\rightarrow$ 自定义 malloc $\rightarrow$ 标准 malloc
...
|包装映射|malloc → __wrap_malloc|将代码中原本调用的 malloc 全部指向你写的 __wrap_malloc。|你的包装函数成功"截胡"。|
|真身还原|__real_malloc → malloc|将你代码中调用的 __real_malloc 指向真正的、原始的 malloc 例程。|保证你的包装器依然能完成实际任务。|
